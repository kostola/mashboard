...
def _settings_to_dict(settings: Settings) -> dict[str, Any]:
    ...
    if settings.tui_button_size is not None:
        payload["tui_button_size"] = settings.tui_button_size
    return payload
...
def _settings_from_dict(data: dict[str, Any]) -> Settings:
    ...
    gui_size = data.get("gui_button_size")
    tui_size = data.get("tui_button_size")
    return Settings(
        output_device=str(device) if device else None,
        monitor_device=str(monitor) if monitor else None,
        gui_button_size=int(gui_size) if gui_size is not None else None,
        tui_button_size=int(tui_size) if tui_size is not None else None,
    )
